    ...
    Multicall
{
    ...
    constructor(address yangNFT, address v3Factory, address deployer) ERC721("YIN's Uniswap V3 Positions Manager", "CHI") {
        _yangNFT = yangNFT;
    ...
    modifier onlyYANG {
        require(msg.sender == address(_yangNFT), "y");
        _;
    }
        ...
        )
    {
        ...
        vault = ICHIVaultDeployer(_deployer).createVault(uniswapPool, address(this), params.vaultFee);
    ...
    function subscribe(
        ...
        uint256 amount0Min,
        uint256 amount1Min
    ) external override onlyYANG subscripting(tokenId) nonReentrant returns (uint256 shares, uint256 amount0, uint256 amount1)
    {
        CHIData storage _chi_ = _chi[tokenId];
    ...
    function unsubscribe(
        ...
        uint256 amount1Min,
        address to
    ) external override onlyYANG nonReentrant returns (uint256 amount0, uint256 amount1)
    {
        bytes32 positionKey = keccak256(abi.encodePacked(yangId, tokenId));
    ...
    function addRange(uint256 tokenId, int24 tickLower, int24 tickUpper) external override isAuthorizedForToken(tokenId) {
        CHIData storage _chi_ = _chi[tokenId];
        ICHIVault(_chi_.vault).addRange(tickLower, tickUpper);
    }

    function removeRange(uint256 tokenId, int24 tickLower, int24 tickUpper) external override isAuthorizedForToken(tokenId) {
        CHIData storage _chi_ = _chi[tokenId];
        ICHIVault(_chi_.vault).removeRange(tickLower, tickUpper);
    }

    function collectProtocol(uint256 tokenId, uint256 amount0, uint256 amount1, address to) external override isAuthorizedForToken(tokenId) {
        CHIData storage _chi_ = _chi[tokenId];
        ICHIVault(_chi_.vault).collectProtocol(amount0, amount1, to);
    }

    function addLiquidityToPosition(uint256 tokenId, uint256 rangeIndex, uint256 amount0Desired, uint256 amount1Desired) external override isAuthorizedForToken(tokenId) {
        CHIData storage _chi_ = _chi[tokenId];
        ICHIVault(_chi_.vault).addLiquidityToPosition(rangeIndex, amount0Desired, amount1Desired);
    }

    function removeLiquidityFromPosition(uint256 tokenId, uint256 rangeIndex, uint128 liquidity) external override isAuthorizedForToken(tokenId) {
        CHIData storage _chi_ = _chi[tokenId];
        ICHIVault(_chi_.vault).removeLiquidityFromPosition(rangeIndex, liquidity);
    }

    function removeAllLiquidityFromPosition(uint256 tokenId, uint256 rangeIndex) external override isAuthorizedForToken(tokenId) {
        CHIData storage _chi_ = _chi[tokenId];
